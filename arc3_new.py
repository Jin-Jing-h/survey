...
class CLIPConditionerExact(nn.Module):
    def __init__(self, clip_out_dim, num_levels=4, feat_dim=512, prompt_dim=32):
        ...
        # --- learnable prompt  ---
        # 4 个分别可训练的 prompt（对应 latent / dec3 / dec2 / dec1）
        self.ctx_latent = nn.Parameter(torch.empty(prompt_dim))
        self.ctx_d3     = nn.Parameter(torch.empty(prompt_dim))
        self.ctx_d2     = nn.Parameter(torch.empty(prompt_dim))
        self.ctx_d1     = nn.Parameter(torch.empty(prompt_dim))
        nn.init.normal_(self.ctx_latent, std=0.02)
        nn.init.normal_(self.ctx_d3,     std=0.02)
        nn.init.normal_(self.ctx_d2,     std=0.02)
        nn.init.normal_(self.ctx_d1,     std=0.02)
        # meta_net（动态 bias）：CoCoOp 的结构 Linear -> ReLU -> Linear
    ...
    def forward(self, clip_feat):
        ...
        bias = self.meta_net(delevel_feat.detach())                  # [B, prompt_dim]
        # p_dynamic = self.ctx.unsqueeze(0) + bias            # [B, prompt_dim]  (CoCoOp 核心)
        p_latent = self.ctx_latent.unsqueeze(0) + bias             # [B, prompt_dim]
        p_d3     = self.ctx_d3.unsqueeze(0)     + bias
        p_d2     = self.ctx_d2.unsqueeze(0)     + bias
        p_d1     = self.ctx_d1.unsqueeze(0)     + bias
        #prompt_vec = p_dynamic.view(-1, self.prompt_dim, 1, 1)


        return p_latent, p_d3, p_d2, p_d1, delevel_feat
...
class Histoformer(nn.Module):
    def __init__(self, 
        ...
        self.cond_to_prompt = nn.Linear(delevel_feat_dim + prompt_dim, prompt_dim, bias=True)
        ...
        self.prompt_dim = prompt_dim
        self.latent_dim = int(dim * 2**3)
        self.dec3_dim   = int(dim * 2**2)
        self.dec2_dim   = int(dim * 2**1)
        self.dec1_dim   = int(dim * 2**1)

        if self.use_prompt:
            if prompt_mode == "add":
                self.prompt_proj_latent = nn.Conv2d(prompt_dim, self.latent_dim, kernel_size=1, bias=False)
                self.prompt_proj_d3     = nn.Conv2d(prompt_dim, self.dec3_dim,   kernel_size=1, bias=False)
                self.prompt_proj_d2     = nn.Conv2d(prompt_dim, self.dec2_dim,   kernel_size=1, bias=False)
                self.prompt_proj_d1     = nn.Conv2d(prompt_dim, self.dec1_dim,   kernel_size=1, bias=False)
            elif prompt_mode == "concat":
                
                fuse_lat = self.latent_dim + self.prompt_dim
                self.prompt_tf_latent   = TransformerBlock(dim=fuse_lat, num_heads=heads[3],
                                                        ffn_expansion_factor=ffn_expansion_factor,
                                                        bias=bias, LayerNorm_type=LayerNorm_type)
                self.prompt_fuse_latent = nn.Conv2d(fuse_lat, self.latent_dim, kernel_size=1, bias=False)

                # --- decoder3 注入 ---
                fuse_d3 = self.dec3_dim + self.prompt_dim
                self.prompt_tf_d3   = TransformerBlock(dim=fuse_d3, num_heads=heads[2],
                                                    ffn_expansion_factor=ffn_expansion_factor,
                                                    bias=bias, LayerNorm_type=LayerNorm_type)
                self.prompt_fuse_d3 = nn.Conv2d(fuse_d3, self.dec3_dim, kernel_size=1, bias=False)

                # --- decoder2 注入 ---
                fuse_d2 = self.dec2_dim + self.prompt_dim
                self.prompt_tf_d2   = TransformerBlock(dim=fuse_d2, num_heads=heads[1],
                                                    ffn_expansion_factor=ffn_expansion_factor,
                                                    bias=bias, LayerNorm_type=LayerNorm_type)
                self.prompt_fuse_d2 = nn.Conv2d(fuse_d2, self.dec2_dim, kernel_size=1, bias=False)

                # --- decoder1 注入 ---
                fuse_d1 = self.dec1_dim + self.prompt_dim
                self.prompt_tf_d1   = TransformerBlock(dim=fuse_d1, num_heads=heads[0],
                                                    ffn_expansion_factor=ffn_expansion_factor,
                                                    bias=bias, LayerNorm_type=LayerNorm_type)
                self.prompt_fuse_d1 = nn.Conv2d(fuse_d1, self.dec1_dim, kernel_size=1, bias=False)
            else:
                raise ValueError("prompt_mode must be 'add' or 'concat'")
        # ---------------- CLIP DeLevel conditioner ----------------
        ...
        if self.use_delevel:
            self.cls_head = nn.Linear(delevel_feat_dim, num_levels-1)
        ...
        if freeze_backbone:
            ...
            if self.use_prompt:
                if self.prompt_mode == "add":
                    for m in [self.prompt_proj_latent, self.prompt_proj_d3,
                              self.prompt_proj_d2, self.prompt_proj_d1]:
                        for p in m.parameters():
                            p.requires_grad = True
                else:
                # concat: 4 个注入点的 prompt_tf / prompt_fuse 都要解冻
                    for m in [self.prompt_tf_latent, self.prompt_fuse_latent,
                            self.prompt_tf_d3, self.prompt_fuse_d3,
                            self.prompt_tf_d2, self.prompt_fuse_d2,
                            self.prompt_tf_d1, self.prompt_fuse_d1]:
                        for p in m.parameters():
                            p.requires_grad = True
            ...
            for p in self.conditioner.parameters():
                p.requires_grad = True
            for p in self.cond_to_prompt.parameters():
                p.requires_grad = True
            # clip_backbone 内部已冻结（保持冻结即可）
        # 解冻：conditioner + cond_to_prompt + cls_head
            # 解冻：4 个注入点的 prompt_tf / prompt_fuse
            if self.use_prompt and self.prompt_mode == "concat":
                self.prompt_tf_latent.requires_grad_(True)
                self.prompt_fuse_latent.requires_grad_(True)

                self.prompt_tf_d3.requires_grad_(True)
                self.prompt_fuse_d3.requires_grad_(True)

                self.prompt_tf_d2.requires_grad_(True)
                self.prompt_fuse_d2.requires_grad_(True)

                self.prompt_tf_d1.requires_grad_(True)
                self.prompt_fuse_d1.requires_grad_(True)

    # clip_backbone 保持冻结即可（它内部本来 requires_grad=False）
    def _inject_prompt(self, feat, prompt_vec, stage="latent"):
        """
        ...
        if self.prompt_mode == "add":
            if stage == "latent":
                return feat + self.prompt_proj_latent(p)
            elif stage == "d3":
                return feat + self.prompt_proj_d3(p)
            elif stage == "d2":
                return feat + self.prompt_proj_d2(p)
            elif stage == "d1":
                return feat + self.prompt_proj_d1(p)
            else:
    ...
    # concat mode
        ...
        if stage == "latent":
            x = self.prompt_tf_latent(x)
            x = self.prompt_fuse_latent(x)
        elif stage == "d3":
            x = self.prompt_tf_d3(x)
            x = self.prompt_fuse_d3(x)
        elif stage == "d2":
            x = self.prompt_tf_d2(x)
            x = self.prompt_fuse_d2(x)
        elif stage == "d1":
            x = self.prompt_tf_d1(x)
            x = self.prompt_fuse_d1(x)
        else:
            raise ValueError(f"Unknown stage={stage}")
    ...
    def forward(self, inp_img, level_gt=None, return_cls=False):
       ...
       #use_gt = (level_gt is not None) and self.training   # 训练用GT，推理不用
        p_latent, p_d3, p_d2, p_d1, delevel_feat= self.conditioner(clip_feat)
        logits = self.cls_head(delevel_feat)
        cond = torch.cat([delevel_feat, p_latent], dim=1) 
        prompt_inj = self.cond_to_prompt(cond)                           # [B, prompt_dim]
        prompt_vec = prompt_inj.view(prompt_inj.size(0), self.prompt_dim, 1, 1)

        cond_d3 = torch.cat([delevel_feat, p_d3], dim=1) 
        prompt_inj = self.cond_to_prompt(cond_d3)                           # [B, prompt_dim]
        prompt_vec_d3 = prompt_inj.view(prompt_inj.size(0), self.prompt_dim, 1, 1)

        cond_d2 = torch.cat([delevel_feat, p_d2], dim=1) 
        prompt_inj = self.cond_to_prompt(cond_d2)                           # [B, prompt_dim]
        prompt_vec_d2 = prompt_inj.view(prompt_inj.size(0), self.prompt_dim, 1, 1)

        cond_d1 = torch.cat([delevel_feat, p_d1], dim=1) 
        prompt_inj = self.cond_to_prompt(cond_d1)                           # [B, prompt_dim]
        prompt_vec_d1 = prompt_inj.view(prompt_inj.size(0), self.prompt_dim, 1, 1)
    ...
    # --- 3) bottleneck prompt injection（严格：只用 conditioner prompt）---
        if self.use_prompt:
            latent = self._inject_prompt(latent, prompt_vec, stage="latent")
        ...
        if self.use_prompt:
            out_dec_level3 = self._inject_prompt(out_dec_level3, prompt_vec_d3, stage="d3")
        ...
        if self.use_prompt:
            out_dec_level2 = self._inject_prompt(out_dec_level2, prompt_vec_d2, stage="d2")
        inp_dec_level1 = self.up2_1(out_dec_level2)
        inp_dec_level1 = torch.cat([inp_dec_level1, out_enc_level1], 1)
        out_dec_level1 = self.decoder_level1(inp_dec_level1)
        if self.use_prompt:
            out_dec_level1 = self._inject_prompt(out_dec_level1, prompt_vec_d1, stage="d1")
