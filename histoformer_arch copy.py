...
class Histoformer(nn.Module):
    def __init__(self, 
        ...
        if self.use_prompt:
            if prompt_mode == "add":
                self.prompt_proj = nn.Conv2d(prompt_dim, self.latent_dim, kernel_size=1, bias=False)
            elif prompt_mode == "concat":
                
                fuse_in_dim = self.latent_dim + prompt_dim
    ...
    def forward(self, inp_img, level_gt=None, return_cls=False):
       ...
       #use_gt = (level_gt is not None) and self.training   # 训练用GT，推理不用
        p_dynamic, delevel_feat = self.conditioner(clip_feat)
        logits = self.cls_head(delevel_feat)
        cond = torch.cat([delevel_feat, p_dynamic], dim=1) 
    ...
    # --- 3) bottleneck prompt injection（严格：只用 conditioner prompt）---
        if self.use_prompt:
            latent = self._inject_prompt(latent, prompt_vec)
