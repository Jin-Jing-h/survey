...
class Dataset_PairedImage(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        if self.opt['phase'] == 'train':
            self.geometric_augs = opt['geometric_augs']
                # ✅ 读取 level_json（可选）
        self.level_map = None
        self.num_levels = opt.get('num_levels', None)
        level_json = opt.get('level_json', None)
    ...
    def __getitem__(self, index):
        ...
        if self.level_map is not None:
            base = os.path.basename(lq_path)          # 308.png
            stem = os.path.splitext(base)[0]          # 308
            key = None
            for k in (stem, base):                    # 先用 "308"，再尝试 "308.png"
                if k in self.level_map:
                    key = k
                    break
            if key is None:
                raise KeyError(f"[level_json] missing key for {base} (also tried stem={stem})")

            lvl = int(self.level_map[key])
            out['level'] = torch.tensor(lvl, dtype=torch.long)
        return out
