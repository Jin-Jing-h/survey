...
def main():
    ...
    while current_iter <= total_iters:
        ...
        epoch_loss_sum = {}   # dict: {loss_name: sum}
        epoch_loss_cnt = 0
...
#                continue
            ...
            if opt['train']['scheduler'].get('type') != 'ReduceLROnPlateau':
                ...
                if len(loss_list) >= 1000:
                    ...
                    if tb_logger and is_rank0:
                        tb_logger.add_scalar('lr/value_for_plateau', float(np.mean(loss_list)), current_iter)
            ...
            lq = train_data['lq']
            gt = train_data['gt']
            label = train_data.get('label', None)  # 没有就 None
            level = train_data.get('level', None)   # ✅ 新增：拿到 level
            if mini_batch_size < batch_size:
                ...
                if label is not None:
                    label = label[indices]          # ✅ 防 None
                if level is not None:
                    level = level[indices]          # ✅ level 也要同步切
            ...
            if label is not None:
                feed['label'] = label
            if level is not None:
                feed['level'] = level   # ✅ 关键：把 level 喂给模型
            model.feed_train_data(feed)

            model.optimize_parameters(current_iter)
            # 统一用 reduce 后的 log_dict（DDP一致）
            loss_items = {}
            for k, v in model.log_dict.items():
                if k.startswith('l_') or k == 'loss_total':
            ...
            # epoch 累计 + 计数（不要放在 if 里）
            for k, v in loss_items.items():
                epoch_loss_sum[k] = epoch_loss_sum.get(k, 0.0) + v
            epoch_loss_cnt += 1
...
#                metric_out = model.metric_results.items()[opt['val']['metrics'].keys()[0]]
                    if metric_out != 0:
                        if tb_logger and is_rank0:
                            tb_logger.add_scalar(f'metrics/psnr_{val_name}', float(metric_out), current_iter)
            data_time = time.time()
            iter_time = time.time()
            train_data = prefetcher.next()
        # end of iter
        if tb_logger and is_rank0 and epoch_loss_cnt > 0:
            for k, s in epoch_loss_sum.items():
                tb_logger.add_scalar(f'losses/{k}_epoch', float(s / epoch_loss_cnt), epoch)
        epoch += 1
